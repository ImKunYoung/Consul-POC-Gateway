...
import com.poc.gateway.config.EmbeddedKafka;
import com.poc.gateway.config.KafkaSseConsumer;
import com.poc.gateway.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class ConsulPocGatewayKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/consul-poc-gateway-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/consul-poc-gateway-kafka/consume")
