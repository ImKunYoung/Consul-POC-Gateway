...
@RestController
@RequestMapping("/api/consul-poc-gateway-kafka")
public class ConsulPocGatewayKafkaResource {

    private final Logger log = LoggerFactory.getLogger(ConsulPocGatewayKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public ConsulPocGatewayKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
